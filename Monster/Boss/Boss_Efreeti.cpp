...
bool Boss_Efreeti::init() {
	...
	baseSprite = Sprite::createWithSpriteFrameName("Boss_Efreeti_0001.png"); // 找图
	addChild(baseSprite);
	cloud = Sprite::createWithSpriteFrameName("Boss_Efreeti_Cloud_0001.png"); // 找图
	...
	createAndSetHpBar(); // 添加血条
	lastState = stateNone;
	scheduleUpdate();
	schedule(schedule_selector(Boss_Efreeti::skill), 20.0f, 2, 5.0f); // 发射技能调度器
	addListener();
	return true;
}
