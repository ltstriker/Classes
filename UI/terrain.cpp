...
{
	auto terrain_instance = new terrain();
	if (terrain_instance && terrain_instance->init(type)) {
		terrain_instance->autorelease();
		return terrain_instance;
	}
	CC_SAFE_DELETE(terrain_instance);
	return NULL;
}
...
bool terrain::init(int type)
{
	...
	{
	case(1): {
		terrain = Sprite::createWithSpriteFrameName("otherresources/build_terrain_0004.png"); }
			 break;
	case(2): {
		terrain = Sprite::createWithSpriteFrameName("otherresources/build_terrain_0005.png"); }
			 break;
	case(3): {
		terrain = Sprite::createWithSpriteFrameName("otherresources/build_terrain_0006.png"); }
			 break;
	default: {
		terrain = Sprite::createWithSpriteFrameName("otherresources/build_terrain_0004.png"); }
			 break;
...
{
	auto smoke = Sprite::createWithSpriteFrameName("otherresources/effect_sellSmoke_0001.png");
	addChild(smoke, 99);
